...
final class Workspace: TreeNode, NonLeafTreeNodeObject, Hashable, Comparable {
    ...
    /// Used only when `config.experimentalStackingLayout == true`; the existing
    ...
    @MainActor
    private var _stackingLayout: StackingLayout? = nil

    @MainActor
    var stackingLayout: StackingLayout {
        if let existing = _stackingLayout { return existing }
        ...
        let layout = StackingLayout(shape: LayoutShape(orientation: orientation, lanes: 6))
        _stackingLayout = layout
        return layout
    }
