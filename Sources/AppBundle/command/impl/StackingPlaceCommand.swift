...
/// `mur stacking-place <lane> <slot0> <slot1>` — place the focused window
/// (or `--window-id <id>`) at the given grid span. Updates
/// `WindowMemory` so reopens of the same app+title come back here.
///
/// Errors:
///  • No experimental-stacking-layout flag → fails with a hint to enable it.
...
struct StackingPlaceCommand: Command {
    let args: StackingPlaceCmdArgs
    /*conforms*/ let shouldResetClosedWindowsCache = false

    func run(_ env: CmdEnv, _ io: CmdIo) -> BinaryExitCode {
        guard config.experimentalStackingLayout else {
            io.err("stacking-place requires `experimental-stacking-layout = true` in mur.toml")
            return .fail
        }
        guard let target = args.resolveTargetOrReportError(env, io) else { return .fail }
        guard let window = target.windowOrNil else {
            io.err("stacking-place needs a focused window or --window-id <id>")
            return .fail
        }
        ...
        StackingHud.shared.update(layout: workspace.stackingLayout, span: span)
